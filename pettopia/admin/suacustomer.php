...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($dobDate >= $currentDate) {
        ...
        // Check if phone number or email already exists for other customers
        $check_sql = "SELECT * FROM customer WHERE (PhoneNumber = '$phone' OR email = '$email') AND IDCustomer != $id";
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            if (mysqli_query($conn, $sql)) {
                $success = 'Chỉnh sửa thông tin khách hàng thành công!';
    ...
    <div class="panel-body">
        <form action="" method="POST" role="form" id="customerForm">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="Name_customer" id="Name_customer" value="<?php echo isset($rowCustomer['Name_customer']) ? $rowCustomer['Name_customer'] : ''; ?>" required>
            </div>
            <div class="form-group">
                <label for="DateOfBirth">Ngày sinh</label>
                <input type="date" class="form-control" name="DateOfBirth" id="DateOfBirth" value="<?php echo isset($rowCustomer['DateOfBirth']) ? $rowCustomer['DateOfBirth'] : ''; ?>" required>
                <div style="color: red; font-size: 1em; display: none;" id="dob-error"></div>
                <?php if ($dobError): ?>
                    <div class="error-message" id="dob-error-server"><?php echo $dobError; ?></div>
                <?php endif; ?>
            </div>
            <div class="form-group">
                <label for="Diachi">Địa chỉ</label>
                <input type="text" class="form-control" name="Diachi" id="Diachi" value="<?php echo isset($rowCustomer['Diachi']) ? $rowCustomer['Diachi'] : ''; ?>" required>
            </div>
            <div class="form-group">
                <label for="PhoneNumber">Số điện thoại</label>
                <input type="text" class="form-control" name="PhoneNumber" id="PhoneNumber" value="<?php echo isset($rowCustomer['PhoneNumber']) ? $rowCustomer['PhoneNumber'] : ''; ?>" required pattern="\d{10}">
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-control" name="email" id="email" value="<?php echo isset($rowCustomer['email']) ? $rowCustomer['email'] : ''; ?>" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var dobErrorServer = document.getElementById('dob-error-server');
    ...
    setTimeout(function() {
        ...
        errorMessages.forEach(function(errorMessage) {
            if (errorMessage.id !== 'dob-error' && errorMessage.id !== 'dob-error-server') {
